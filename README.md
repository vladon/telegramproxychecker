...
| `--auth-session DIR` | TDLib `database_directory` (created if missing; files under `DIR/tg-proxy-check-files/`). Runs stdin login (**phone** / **code** / **2FA**) until TDLib is ready when needed; session persists on disk. After MTProto **`pingProxy`** success, runs sponsor detection (see below); optional. |
...
**`sponsored`** and **`subscription`** are always present. Without **`--auth-session`**, `sponsored.status` is **`unknown`**, `subscription.checked` is **`false`**, and no sponsor calls are made. With **`--auth-session DIR`** and **MTProto** success, after **`authorizationStateReady`** the tool runs **`loadChats`** (main list), **`getChats`**, then **`getChat`** for each listed id (up to 200) and treats **`sponsored.yes`** only when TDLib marks a **channel** chat’s **`positions[].source`** as **`chatSourceMtprotoProxy`** (see TDLib `td_api.tl`). If every **`getChat`** completes and none match, **`sponsored.no`**. If RPCs fail, time out, or some **`getChat`** replies are missing before the probe deadline, **`sponsored.unknown`** and **`subscription.checked`** is **`true`**. For **`yes`**, **`channel_id`** is the supergroup’s numeric id; **`getMe`** + **`getChatMember`** set **`subscription.joined`** when TDLib returns **`chatMember`**. SOCKS5 does not run this path (`checked` stays **`false`**).
...
Pinned revision: **upstream `master` snapshot** (commit `8ff05a0e7e064fa796593f3105c2dcf983e279d4`), defined as `TD_COMMIT` in `build.rs` and mirrored in **`ci/tdlib-build-fingerprint`** (CI checks they match). After bumping TDLib, update **both** files, then remove the old cache under **`target/tdlib-build-cache/cmake/<old-commit>/`** (or run **`cargo clean`**); the new commit uses a separate subtree automatically.

### Where native artifacts go

| Stage | Location (under **`target/`** or **`$CARGO_TARGET_DIR`**) |
|--------|------------------------------------------------------------------|
| Per-target CMake + install | `tdlib-build-cache/cmake/<TD_COMMIT>/<Rust TARGET>/<profile>/tdlib-cmake/` and `.../tdlib-install/lib/` — **profile** is `unix-static` (glibc Linux / typical Unix), `musl-shared`, `musl-static`, `macos`, or `windows` (see `td_cmake_cache_segment` in `build.rs`). |
| Tarball extract (if used) | `tdlib-build-cache/source/<TD_COMMIT>/td-<commit>/` (shared; **build trees** stay per `TARGET` + profile) |

**`TDLIB_BUILD_VARIANT`** is a free-form label for CI/logging; it does **not** choose the CMake folder unless the name contains **`musl-static`** or **`musl-v3-static`** (that switches the **musl-static** CMake path). Generic vs **x86-64-v3** rows reuse the same TDLib tree when **`TARGET`** and profile match. To force a separate CMake tree (e.g. experiments), set **`TDLIB_CMAKE_CACHE_ID`** to a unique slug.
...
The first build compiles TDLib and can take **several minutes** and several GB under `target/`. Later `cargo build` runs are incremental: CMake + Ninja/Make reuse **`target/tdlib-build-cache/cmake/...`** even when Cargo’s `build/tg-proxy-check-*` hash changes, until TDLib sources, **`TARGET`**, CMake profile (see table above), **`TDLIB_CMAKE_CACHE_ID`**, or env vars that change the CMake step change.
...
**Faster Release CI (optional):** maintain a long-lived GitHub Release tagged **`tdlib-prebuilt`** with assets `tdlib-gnu-<TD_COMMIT>.tar.zst` and `tdlib-musl-static-<TD_COMMIT>.tar.zst` (zstd tar of `target/tdlib-build-cache`). The **`prep-tdlib`** job downloads them when the URL exists so TDLib often does not compile from source. Publish or refresh those assets with the manual workflow [`.github/workflows/tdlib-prebuilt.yml`](.github/workflows/tdlib-prebuilt.yml) after bumping the TD pin.

Workflows live under [`.github/workflows/`](.github/workflows/): **`release.yml`** runs the full release matrix on matching tags; shared Ubuntu/Rust/cross/cache steps live in the composite action [`.github/actions/release-build-env/action.yml`](.github/actions/release-build-env/action.yml). **`ci.yml`** runs lightweight `fmt` / `clippy` / `cargo test --no-default-features` on pushes and PRs to `main` or `master` only (no TDLib compile). The release workflow runs **`prep-tdlib`** first (two parallel legs: **gnu**, **musl static**). Each leg runs **`cargo` / `cross build`** once to populate TDLib, then uploads **`target/tdlib-build-cache`** as a workflow artifact (`tdlib-prep-*`, short retention). The **`build`** matrix downloads the matching artifact so **two** gnu or **two** musl-static rows in the same run do not each compile TDLib from scratch. **`actions/cache`** still warms **`~/.cargo/registry`** and TDLib across workflow runs. All **four** Linux x86_64 variants share one **`build`** matrix job (**fail-fast off** so other legs keep running if one fails). **Publish** runs after the matrix finishes (tag push or **workflow_dispatch**) and uploads **whatever artifacts succeeded**. Tag pushes attach assets to the normal release for that tag. **Manual runs** create a **draft** GitHub Release named `CI <version_label> (run …)` with tag `ci-<version_label>-<run_id>` on the workflow commit (if every leg fails before upload, a warning is emitted and no release is created). **Musl** matrix rows use [**cross-rs**](https://github.com/cross-rs/cross) on Ubuntu (glibc-hosted Cargo with `--target x86_64-unknown-linux-musl`) because a **musl-only** Rust host cannot build procedural macros such as `clap_derive` (see [`Cross.toml`](Cross.toml) and [`docker/cross-musl.Dockerfile`](docker/cross-musl.Dockerfile)). **Release tags** must point at a commit that includes that Dockerfile-based setup (older tags that only used a `pre-build` script hook will fail in CI).
...
- **musl / Alpine:** A normal **`cargo build`** with the default musl target can link the **shared** `libtdjson.so` from `tdlib-install/lib` and set **rpath** to that directory. **GitHub Release** archives for musl ship **fully static** binaries only (`*-musl-static*`). Do not strip `target/` if you rely on that path for local shared builds, or ship `libtdjson.so` next to your binary and adjust the loader.
