...
Workflows live under [`.github/workflows/`](.github/workflows/): **`release.yml`** runs the full release matrix on matching tags; **`ci.yml`** runs lightweight `fmt` / `clippy` / `cargo test --no-default-features` on pushes and PRs to `main` or `master` only (no TDLib compile). All six variants share one **`build`** matrix job (**fail-fast off** so other legs keep running if one fails). On **tag** pushes, **Publish** runs after the matrix finishes and uploads **whatever artifacts succeeded** to the GitHub Release (if every leg fails, no assets are attached and a warning is emitted). **Musl** matrix rows use [**cross-rs**](https://github.com/cross-rs/cross) on Ubuntu (glibc-hosted Cargo with `--target x86_64-unknown-linux-musl`) because a **musl-only** Rust host cannot build procedural macros such as `clap_derive` (see [`Cross.toml`](Cross.toml) and [`docker/cross-musl.Dockerfile`](docker/cross-musl.Dockerfile)). **Release tags** must point at a commit that includes that Dockerfile-based setup (older tags that only used a `pre-build` script hook will fail in CI).
